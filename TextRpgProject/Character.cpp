...
int Character::CharacterAttack()
{
	...
	if (randomScope < criticalChance)
	{
		...
		LogBox::GetInstance()->Print(format("{} 님의 공격이 {} 만큼 치명적인 데미지를 주었습니다!", name, attack),RGB(255,128,0));
	}
	else {
...
{
	LogBox::GetInstance()->Print(format("========== {} 의 인벤토리 ==========", name), RGB(0, 255, 255));
	//std::cout << "\n===== " << name << "의 인벤토리 =====\n";
	...
	for (const auto& pair : inventory) {
		...
		(
			format("[ {} ] {} (체력 + {}, 공격력 + {}) x {} 개",
				index++, item->GetName(), item->GetHealthBoost(), item->GetAttackBoost(), quantity
			), RGB(0, 255, 255));
	}
}

void Character::DisplayItemList()
{
	std::cout << "\n===== " << name << "의 인벤토리 =====\n";
...
int Character::UseItem()
{
	...
	while (true)
	{
		if (inventory.empty())
		{
			LogBox::GetInstance()->Print("현재 보유하고 있는 아이템이 없습니다.");
		...
		cout << "선택: ";
		*/
		LogBox::GetInstance()->Print("[ 0 ] 사용안함.");
		LogBox::GetInstance()->Print("사용하실 아이템을 선택해주세요.");
		...
		if (!item) {
			LogBox::GetInstance()->Print("잘못된 아이템 번호입니다.", RGB(255, 0, 0));
		...
		else
		{
			...
			LogBox::GetInstance()->Print(format("[ {} ] 은 {} 아이템을 사용하였습니다!", name, item->GetName()),RGB(255,128,0));
...
{
	LogBox::GetInstance()->Print(format("캐릭터가 {} 만큼 회복되었습니다.", health));
...
{
	LogBox::GetInstance()->Print(format("캐릭터의 공격력이 {} 만큼 상승하였습니다.", attackBoost));
...
void Character::DisplayStatus() const {
	/*
	::GetInstance()->Print(format("===== {} 의 상태 =====", name));
	LogBox::GetInstance()->Print(format("체력: {} / {}", hp / maxhp));
	LogBox::GetInstance()->Print(format("마나: {} / {}", hp / maxhp));
	LogBox::GetInstance()->Print(format("공격력: {}", attack));
	LogBox::GetInstance()->Print(format("골드: {}", gold));
	*/
...
void Character::PrintSkillList()
{
	...
	for (const auto& item : skillInventory)
	{
		LogBox::GetInstance()->Print
		(
			format("[{}] {} / 데미지: {} / 소비마나: {}", index, item->GetSkillName(), item->GetDamage(), item->GetMp())
		);
		index++;
	}
}
...
{
	LogBox::GetInstance()->Print(format("{} 님이 {} 스킬을 얻었습니다!", name, skill->GetSkillName()), RGB(255, 128, 128));

	if (skillInventory.size() >= 3) {
		LogBox::GetInstance()->Print("더이상 스킬을 얻을 수 없습니다.", RGB(192,0,0));
		Sleep(2000);
	}
...
int Character::UseSkill()
{
	...
	while (true)
	{
		...
		{
			LogBox::GetInstance()->Print("현재 사용할 수 있는 스킬이 없습니다.", RGB(192, 0, 0));
			Sleep(1000);
		...
		PrintSkillList();
		LogBox::GetInstance()->Print("[ 0 ] 사용안함.");
		LogBox::GetInstance()->Print("사용하실 스킬을 선택해주세요.");
		choice = InputBox::GetInstance()->InputNumber();
		...
		if (skill != nullptr)
		{
			...
			if (mp < skill->GetMp())
			{
				LogBox::GetInstance()->Print("스킬에 필요한 마나가 부족합니다.", RGB(192, 0, 0));
				break;
			}

			damage = skill->GetDamage() + static_cast<int>(skill->GetDamage() * skillEnhancement / 100);
			LogBox::GetInstance()->Print(format("[{}] 님의 {} !", name, skill->GetSkillName()), RGB(255, 128, 0));
			mp -= skill->GetMp();
		...
		{
			LogBox::GetInstance()->Print("스킬이 존재하지 않습니다.", RGB(192, 0, 0));
		}
	}
